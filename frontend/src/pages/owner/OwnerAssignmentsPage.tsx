...
import { CheckCircle, Clock, AlertCircle, Search, Eye, Plus } from 'lucide-react'
...
export default function OwnerAssignmentsPage() {
  ...
  const stats = {
    ...
    inProgress: assignments.filter(a => a.status?.toUpperCase() === 'IN_PROGRESS').length,
  }
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-2">Manage and review all employee assignments</p>
        </div>
        <button
          onClick={() => navigate('/owner/assignments/create')}
              ...
              <tbody className="divide-y divide-slate-200">
                {filteredAssignments.map(assignment => (
                  <tr key={assignment.assignment_id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm text-slate-600">Employee #{assignment.assigned_to_user_id}</span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <button
                        onClick={() => navigate(`/owner/assignments/${assignment.assignment_id}`)}
